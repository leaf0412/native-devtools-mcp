//! CDP wait tools: `cdp_wait_for` (page-text polling) and
//! `cdp_wait_for_page_change` (semantic-delta wait, optionally element-scoped).
//!
//! Wait quadrants are realised as free functions
//! (`wait_page_text`, `wait_page_semantic`, `wait_scoped_semantic`) behind
//! thin public entry points. See ADR 0004 for why per-quadrant fns won out
//! over a `WaitStrategy` trait.

use super::scope::{
    borrow_page, json_value_arg, release_object, resolve_element_scope, ResolvedScope,
};
use super::summary::cdp_take_dom_snapshot;
use crate::cdp::{cdp_error, CdpClient};
use chromiumoxide::cdp::js_protocol::runtime::{
    CallFunctionOnParams, EvaluateParams, ExceptionDetails, RemoteObjectId,
};
...
/// Extract a JSON value from a CDP execute response, treating any
/// JavaScript exception as a tool error with the canonical
/// "JavaScript exception while waiting for page change:" prefix.
fn extract_semantic_value(
...
async fn wait_page_semantic(
    ...
    extract_semantic_value(
        resp.result.exception_details.as_ref(),
        resp.result.result.value.as_ref(),
    )
}

async fn wait_scoped_semantic(
    page: &Page,
    ...
    // Release the remote object unconditionally before surfacing any error
    // — must come after the await, regardless of outcome. Drop magic cannot
    // be used here because Drop is not async.
...
/// Normalised inputs for `cdp_wait_for_page_change`. All clamping happens
/// in `from_raw`; the rest of the wait pipeline assumes already-normalised
/// values.
#[derive(Debug, Clone)]
pub struct SemanticWaitParams {
    /// `None` after trim; never `Some("")` or `Some("   ")`.
    pub scope_uid: Option<String>,
    /// Clamped to `<= MAX_PAGE_CHANGE_WAIT_TIMEOUT_MS`.
    pub timeout_ms: u64,
    /// Clamped to `[MIN_PAGE_CHANGE_POLL_MS, MAX_PAGE_CHANGE_POLL_MS]`.
    pub poll_interval_ms: u64,
    /// Clamped to `[MIN_PAGE_CHANGE_STABLE_MS, MAX_PAGE_CHANGE_STABLE_MS]`.
    pub stable_ms: u64,
    /// Defaults to `"semantic_delta"` when absent.
    pub condition: String,
    pub goal: Option<String>,
    pub include_snapshot: bool,
}

impl SemanticWaitParams {
    /// Apply defaults, clamps, and trim semantics. Pure; the only meaningful
    /// logic in the public entry — unit-testable without a CDP mock.
    pub fn from_raw(
...
/// Wait quadrant chosen by `select_strategy`. Page-text polling lives in
/// `cdp_wait_for` and is not part of this enum (it's a different public tool
...
/// Pure dispatch decision: returns which quadrant the semantic wait should
/// run. Separated so the boundary stays unit-testable.
fn select_strategy(params: &SemanticWaitParams) -> WaitQuadrant {
    match params.scope_uid.as_deref() {
        Some(_) => WaitQuadrant::Scoped,
        None => WaitQuadrant::Page,
    }
}
    ...
    match select_strategy(params) {
        ...
        WaitQuadrant::Scoped => {
            let uid = params
                .scope_uid
                .as_deref()
                .expect("Scoped quadrant requires scope_uid; select_strategy enforces this");
            let ResolvedScope { object_id, .. } =
                resolve_element_scope(uid, page, cdp_client).await?;
            wait_scoped_semantic(
                page,
                object_id,
                params.timeout_ms,
                params.stable_ms,
...
pub async fn cdp_wait_for_page_change(
    ...
    let value = match run_semantic(&page, &params, &cdp_client).await {
        Ok(value) => value,
        Err(e) => return e,
    };

    let result = decorate_semantic_wait_result(
        value,
        params.scope_uid.as_deref(),
        &params.condition,
